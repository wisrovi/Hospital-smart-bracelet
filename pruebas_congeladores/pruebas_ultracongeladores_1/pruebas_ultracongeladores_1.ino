...
void SENSORES1(AsyncWebServerRequest *request) {//empleado 2 toma dato de forma individual del sensor DHT22
  ...
  const char* VALOR_VARIABLE_BUSCAR = "Umbral_DHT22";
...
void SENSORES2(AsyncWebServerRequest *request) {   //empleado 3 toma dato de forma individual del sensor DS18B20
  ...
  ProcesarClienteRequest(request);    //only for AP
  const char* VALOR_VARIABLE_BUSCAR = "Umbral_Minimo_DS18B20";
  if (request->hasParam(VALOR_VARIABLE_BUSCAR)) {
    String valor_variable = request->getParam(VALOR_VARIABLE_BUSCAR)->value();
...
void SENSORES3(AsyncWebServerRequest *request) {   //empleado 4 toma dato de forma individual del sensor Termocupla
  ...
  ProcesarClienteRequest(request);    //only for AP
  const char* VALOR_VARIABLE_BUSCAR = "Umbral_Minimo_Termocupla";
  if (request->hasParam(VALOR_VARIABLE_BUSCAR)) {
    String valor_variable = request->getParam(VALOR_VARIABLE_BUSCAR)->value();
...
void setup() {
  ...
  //***SERVICIO AP
  if (isActiveUseAP()) {
    if (ConfigWifiAP("KITARA86", "TOLOSA86"
    ) == false) {
  ...
  if (getStatusConectionwifi() && isActiveUseWifi()) {
    ...
    setIniciarServidorAP(); //servicio AP
  }
